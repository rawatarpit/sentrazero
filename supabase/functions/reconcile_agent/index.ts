...
serve(async (req)=>{
  ...
  const supabase = createClient(SUPABASE_URL, SERVICE_KEY, {
    global: {
      headers: {
        "x-client-info": "reconcile_agent/1.0.0"
      }
    }
  });
  const { data: staleJobs, error: staleErr } = await supabase.from("agent_jobs").update({
    status: "pending",
    agent_id: null,
    lease_expires_at: null
  }).eq("agent_id", deviceId).eq("org_id", orgId).in("status", [
    "assigned",
    "running"
  ]).lt("lease_expires_at", new Date().toISOString()).select("id");
  if (staleErr) {
    console.error("[reconcile_agent] Failed to restore stale jobs:", staleErr.message);
    return jsonResponse({
      ok: false,
      error: "Failed to restore stale jobs: " + staleErr.message
    }, 500, corsHeaders);
  }
  const restoredCount = staleJobs?.length ?? 0;
  await supabase.from("devices").update({
    last_refresh: new Date().toISOString()
  }).eq("id", deviceId).eq("org_id", orgId);
  return jsonResponse({
    ok: true,
