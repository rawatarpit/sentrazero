...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[auto_assign_best_device] ✅ Selected device ${selectedDeviceId}` +
      ` (score: ${best.score?.toFixed(3) ?? "n/a"})`
    );

    const { data: device } = await supabase
      .from("devices")
      .select(
        "id, name, active_jobs, max_concurrency, cpu_cores_free, total_cpu_cores, memory_free_gb, total_memory_gb"
      )
      .eq("id", selectedDeviceId)
      .single();

    if (device) {
      const cpuRatio     = device.cpu_cores_free != null && device.total_cpu_cores
        ? (device.cpu_cores_free / device.total_cpu_cores)
        : null;
      const memRatio     = device.memory_free_gb != null && device.total_memory_gb
        ? (device.memory_free_gb / device.total_memory_gb)
        : null;
      const concurRatio  = device.max_concurrency
        ? Math.max(0, (device.max_concurrency - (device.active_jobs ?? 0)) / device.max_concurrency)
        : null;

      console.log(
        `[auto_assign_best_device] 📊 Device ${device.name} —` +
        ` cpu: ${cpuRatio != null ? (cpuRatio * 100).toFixed(1) + "%" : "n/a"}` +
        ` memory: ${memRatio != null ? (memRatio * 100).toFixed(1) + "%" : "n/a"}` +
        ` concurrency: ${concurRatio != null ? (concurRatio * 100).toFixed(1) + "%" : "n/a"}`
      );
    ...
    const relayPayload = {
      ...
      data: { type: "process_dataset", dataset_id, org_id: dataset.org_id, job_type },
    };
    ...
    if (relayContentType.includes("application/json")) {
      ...
      relayData = { raw: await relayRes.text(), status: relayRes.status };
    }
    ...
    const { error: datasetUpdateErr } = await supabase
      .from("datasets")
    ...
    const cpuRatio    = device?.cpu_cores_free != null && device?.total_cpu_cores
      ? device.cpu_cores_free / device.total_cpu_cores : null;
    const memRatio    = device?.memory_free_gb != null && device?.total_memory_gb
      ? device.memory_free_gb / device.total_memory_gb : null;
    const concurRatio = device?.max_concurrency
      ? Math.max(0, (device.max_concurrency - (device.active_jobs ?? 0)) / device.max_concurrency)
      : null;
