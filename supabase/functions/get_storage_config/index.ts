...
serve(async (req) => {
  ...
  try {
    ...
    const body: GetStorageConfigRequest = await req.json();
    const { org_id, storage_config_id } = body;

    console.log("[get_storage_config] received", { org_id, storage_config_id });

    if (!org_id) {
      return response({ ok: false, error: "MISSING_ORG_ID" }, 400);
    }

    console.log("[get_storage_config] stage: fetch_config", { org_id, storage_config_id });
    ...
    if (!config) {
      return response({ ok: false, error: "STORAGE_CONFIG_NOT_FOUND" }, 404);
    }
    ...
    if (config.vault_secret_name) {
      ...
      if (decryptError) {
        ...
        try {
          credentials = typeof decrypted === "string"
    ...
    console.error("[get_storage_config] stage: crash", { error: String(error) });
    return response({
      ok: false,
