...
serve(async (req) => {
  ...
  try {
    ...
    if (!claim_code || typeof claim_code !== 'string' || claim_code.trim() === '') {
      return jsonResponse({ ok: false, error: "Missing or invalid claim_code field" }, 400, corsHeaders);
    }
    ...
    const { data: orgId, error: validationError } = await supabase.rpc(
      "validate_claim_secret",
      { p_claim: claim_code }
    );

    if (validationError) {
      console.error("[claim_device] RPC validation error:", validationError.message, validationError.code);
      return jsonResponse({ ok: false, error: "Claim validation failed: " + validationError.message }, 500, corsHeaders);
    }
    ...
    if (orgError) {
      ...
      return jsonResponse({ ok: false, error: "Failed to fetch organization" }, 500, corsHeaders);
    }

    if (!org) {
      return jsonResponse({ ok: false, error: "Organization not found" }, 404, corsHeaders);
    }
    ...
    const { data: device, error: insertErr } = await supabase
      .from("devices")
      .upsert(devicePayload, { onConflict: "org_id,name" })
      .select("id, org_id, name, status")
      .maybeSingle();

    if (insertErr) {
      console.error("[claim_device] Upsert Error:", insertErr.message, insertErr.code);
      return jsonResponse({ ok: false, error: "Failed to upsert device: " + insertErr.message }, 500, corsHeaders);
    }

    if (!device) {
      console.error("[claim_device] Upsert succeeded but no device returned");
      return jsonResponse({ ok: false, error: "Device upsert failed - no device returned" }, 500, corsHeaders);
    }
