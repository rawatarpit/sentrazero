...
serve(async (req) => {
  ...
  const authResult = await authenticateDeviceWithDetails(
    req,
    "id, org_id, name, total_cpu_cores, total_memory_gb, max_concurrency, last_policy_update"
  );
  ...
  const supabase = createClient(SUPABASE_URL, SERVICE_KEY, {
    global: { headers: { "x-client-info": "agent_health_policy/3.1.0" } },
  });
  ...
  const lastUpdate = device.last_policy_update
    ? new Date(device.last_policy_update).getTime()
    : 0;
  ...
  const totalCores = Number(
    body.total_cpu_cores ?? device.total_cpu_cores ?? 2
  );

  const totalMemory = Number(
    body.total_memory_gb ?? device.total_memory_gb ?? 4
  );
  ...
  const status =
    metrics.cpu_usage_percent > 90
      ? "busy"
      : metrics.memory_free_gb < totalMemory * 0.2
      ? "busy"
      : "available";
  ...
  const memRatio =
    metrics.total_memory_gb > 0
      ? metrics.memory_free_gb / metrics.total_memory_gb
      : 0.5;
