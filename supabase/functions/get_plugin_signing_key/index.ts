...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: signingKey, error } = await supabase.from("plugin_signing_keys").select("id, org_id, public_key, algorithm, created_at, expires_at, revoked_at").eq("id", keyId).eq("org_id", authResult.device.org_id).eq("revoked_at", null).maybeSingle();
    if (error) throw error;
    if (!signingKey) {
      return new Response(JSON.stringify({
        error: "Signing key not found or access denied"
      }), {
        status: 404,
        headers: corsHeaders
      });
