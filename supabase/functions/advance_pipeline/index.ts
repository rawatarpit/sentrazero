...
serve(async (req) => {
  ...
  try {
    ...
    const { data: execution, error: execErr } = await supabase
      .from("executions")
      .select("id, org_id, dataset_id, current_step_index, total_steps, status")
      .eq("id", execution_id)
    ...
    if (execution.status !== "running") {
      return ok({ message: "Execution is not running", execution_id }, corsHeaders);
    }
    ...
    const { data: unfinishedJobs, error: jobsErr } = await supabase
      .from("agent_jobs")
      .select("id")
      .eq("execution_id", execution_id)           // FIX 2: execution_id is a real column — use .eq() directly
    ...
    if ((unfinishedJobs ?? []).length > 0) {
      return ok(
        { message: "Current step still in progress", execution_id, step_index: stepIndex },
        corsHeaders
      );
    }
    ...
    if (nextStepIndex >= execution.total_steps) {
      ...
      return ok({ message: "Execution completed", execution_id }, corsHeaders);
    }
    ...
    const { data: stepDefs, error: stepsErr } = await supabase
      .from("execution_steps")
      .select("step_index, step_type, plugin_id, script_id, config")
      .eq("execution_id", execution_id)
      .order("step_index");
    ...
    return ok(
      { message: "Advanced to next step", execution_id, step_index: nextStepIndex },
      corsHeaders
    );
  } catch (err) {
...
function ok(payload: Record<string, any>, corsHeaders: Record<string, string>) {
  return new Response(JSON.stringify({ ok: true, ...payload }), {
    headers: { "Content-Type": "application/json", ...corsHeaders },
  });
}

function edgeError(message: string, corsHeaders: Record<string, string>) {
