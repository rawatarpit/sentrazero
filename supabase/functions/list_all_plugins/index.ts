...
serve(async (req) => {
  ...
  try {
    ...
    if (builtInError) {
      return new Response(
        JSON.stringify({ error: "Failed to fetch built-in plugins", details: builtInError.message }),
    ...
    if (orgId) {
      const { data: orgPluginRows, error: orgError } = await supabase
        .from("org_plugins")
    ...
    const builtInMarked = (builtInPlugins || []).map((p: any) => ({
      ...p,
    ...
    const orgMarked = orgPlugins.map((p: any) => ({
      ...p,
      plugin_source: "org",
      org_plugin_id: p.id,
    }));

    const allPlugins = [...builtInMarked, ...orgMarked];

    return new Response(
      JSON.stringify({ plugins: allPlugins }),
