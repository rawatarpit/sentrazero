...
serve(async (req)=>{
  ...
  try {
    ...
    console.log("[list_plugin_signing_keys] stage: fetch", {
      orgId
    });
    const { data: signingKeys, error: fetchError } = await supabase.from("plugin_signing_keys").select("id, public_key, algorithm, created_at, expires_at, revoked_at").eq("org_id", orgId).is("revoked_at", null).order("created_at", {
      ascending: false
    });
    if (fetchError) {
    ...
    const activeKeys = (signingKeys || []).filter((key)=>!key.expires_at || new Date(key.expires_at) > new Date());
    ...
    console.error("[list_plugin_signing_keys] stage: crash", {
      error: String(error)
    });
    return response({
      ok: false,
