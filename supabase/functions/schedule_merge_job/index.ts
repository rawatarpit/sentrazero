...
serve(async (req) => {
  ...
  const authResult = await authenticateDeviceWithDetails(
    req,
    "id, org_id, name, merge_capable, status, memory_free_gb, active_workers, max_concurrency"
  );
  ...
  try {
    ...
    const { data: chunks, error: chunksError } = await supabase
      .from("batch_chunks")
      .select("id, chunk_index, status, merged_in, chunk_size_gb")
    ...
    const activeChunks = (chunks || []).filter((c: any) => 
      c.status === "completed" && !c.merged_in
    );
    ...
    const totalSizeGB = dataset.total_size_gb || 
      activeChunks.reduce((sum: number, c: any) => sum + (c.chunk_size_gb || 0), 0);

    const useTreeMerge = activeChunks.length > MAX_CHUNK_COUNT_THRESHOLD || 
      totalSizeGB > DATASET_SIZE_GB_THRESHOLD;

    let targetDevice = authResult.device;

    if (dataset.affinity_device_id && dataset.affinity_device_id !== targetDevice.id) {
      const { data: affinityDevice, error: affinityError } = await supabase
        .from("devices")
        .select("id, name, merge_capable, status, memory_free_gb, active_workers, max_concurrency")
        .eq("id", dataset.affinity_device_id)
        .eq("merge_capable", true)
        .eq("status", "online")
        .maybeSingle();
    ...
    const VALID_MERGE_STATUSES = ["online", "available"];
    if (!targetDevice.merge_capable || !VALID_MERGE_STATUSES.includes(targetDevice.status)) {
      const { data: availableDevices, error: devicesError } = await supabase
        ...
        .eq("status", "online")
        .lt("active_workers", 1)
        .order("memory_free_gb", { ascending: false })
        .limit(10);
    ...
    const mergeJobPayload = {
      ...
      chunks: activeChunks.map((c: any, idx: number) => ({
        chunk_id: c.id,
