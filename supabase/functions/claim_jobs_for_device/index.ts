...
serve(async (req) => {
  ...
  try {
    ...
    if (!jobs || jobs.length === 0) {
      return jsonResponse({ ok: true, jobs: [], message: "No jobs available" }, 200, corsHeaders);
    }

    // Normalize the response to use consistent field names that the agent expects:
    // - id (not job_id)
    // - execution_id (not exec_id)
    // - payload (not job_payload)
    ...
    return jsonResponse({ ok: true, jobs: normalizedJobs }, 200, corsHeaders);
  } catch (err) {
