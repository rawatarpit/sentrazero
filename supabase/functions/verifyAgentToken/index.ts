...
export async function verifyAgentToken(req: Request) {
  ...
  if (!agentToken) {
    return { error: "Missing x-agent-token header" }
  }
  ...
  const { data: device, error: dbError } = await supabase
    .from("devices")
    .select("id, org_id, name, status, revoked_at")
  ...
  if (dbError) {
    ...
    return { error: "Auth query failed" }
  }
  ...
  if (device.status === null || device.status === undefined) {
    ...
    return { error: "Device missing status (DB corruption)" }
  }
