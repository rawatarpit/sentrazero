...
serve(async (req) => {
  ...
  try {
    ...
    const assignRes = await fetch(
      `${Deno.env.get("SUPABASE_URL")}/functions/v1/auto_assign_best_device`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        ...
        body: JSON.stringify({ org_id: authResult.orgId, dataset_id, job_type: "process" }),
      }
    );
    ...
    return new Response(
      JSON.stringify({ ok: true, dataset_id }),
      { headers: corsHeaders }
    );
  } catch (err) {
    return new Response(
