...
interface DecryptVaultSecretRequest {
  secret_name: string;
  org_id?: string;
}
...
serve(async (req) => {
  ...
  try {
    ...
    const body: DecryptVaultSecretRequest = await req.json();
    const { secret_name, org_id } = body;

    console.log("[decrypt_vault_secret] stage: received", { secret_name, org_id });

    if (!secret_name) {
      return response({ ok: false, error: "MISSING_SECRET_NAME" }, 400);
    }

    console.log("[decrypt_vault_secret] stage: fetch", { secret_name });

    const { data: encryptedData, error: rpcError } = await supabase.rpc(
      "decrypt_vault_secret",
      { secret_name: secret_name }
    );

    if (rpcError) {
      console.error("[decrypt_vault_secret] stage: fetch failed", { error: rpcError.message });
      return response({
        ok: false,
    ...
    if (!encryptedData) {
      return response({ ok: false, error: "SECRET_NOT_FOUND" }, 404);
    }

    let decrypted: string | unknown = encryptedData;

    console.log("[decrypt_vault_secret] stage: parse", { dataType: typeof encryptedData });
    ...
    const secret = decrypted as DecryptedSecret;
    ...
    console.log("[decrypt_vault_secret] stage: complete", { provider: secret.provider });
    ...
    console.error("[decrypt_vault_secret] stage: crash", { error: String(error) });
    return response({
      ok: false,
