...
serve(async (req) => {
  ...
  try {
    ...
    const { data: history, error: histErr } = await supabase
      ...
      .select("throughput, latency_ms")
      .eq("device_id", authResult.device.id)
      .order("created_at", { ascending: false })
      .limit(10);
    if (histErr) throw histErr;

    const avgThroughput =
      history.reduce((s, b) => s + (b.throughput || 0), 0) /
      (history.length || 1);
    const avgLatency =
      history.reduce((s, b) => s + (b.latency_ms || 0), 0) /
      (history.length || 1);
    ...
    const blended =
      oldVec && oldVec.length === 16
        ? oldVec.map((v: number, i: number) =>
            +(v * 0.8 + baseVec[i] * 0.2).toFixed(6)
          )
        : baseVec;
    ...
    return new Response(
      JSON.stringify({
        ok: true,
        message:
          "Benchmark recorded, chunk size optimized, and profile vector updated",
        preferred_chunk_size_gb: newChunkSize,
