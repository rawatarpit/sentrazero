...
function generateClaimCode(): string {
  ...
  for (let i = 0; i < CLAIM_CODE_LENGTH; i++) {
    result += CLAIM_CODE_CHARS.charAt(
      Math.floor(Math.random() * CLAIM_CODE_CHARS.length)
    );
  }
  return result;
}
...
serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      while (!orgId && attempts < 5) {
        ...
        const orgRes = await supabase
          ...
          .insert({
            ...
            claim_secret_expires_at: new Date(
              Date.now() + 1000 * 60 * 60 * 24 // 24h expiry
            ).toISOString(),
          })
          .select("id")
          .single();
       ...
       * ------------------------------- */
      const platformPubKey = Deno.env.get(
        "PLATFORM_SIGNING_PUBLIC_KEY_B64"
      );

      if (!platformPubKey) {
        throw new Error(
          "PLATFORM_SIGNING_PUBLIC_KEY_B64 is not set."
        );
      }
      ...
      if (keyRes.error) {
        throw new Error(
          "[create-user] Failed to insert signing key: " +
            keyRes.error.message
        );
      }

      console.log(
        "[create-user] org + claim code created " + orgId
      );

      return new Response(
        JSON.stringify({ user_id: userId, org_id: orgId }),
        { status: 200, headers }
      );
    } catch (err) {
