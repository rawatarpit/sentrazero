...
serve(async (req) => {
  ...
  try {
    ...
    console.log("[list_plugins_for_org] received", { org_id });

    if (!org_id) {
      return response({ ok: false, error: "MISSING_ORG_ID" }, 400);
    }

    console.log("[list_plugins_for_org] stage: fetch_builtin");

    const { data: builtInPlugins, error: builtInError } = await supabase
      .from("plugins")
      .select("*")
      .eq("plugin_group", "builtin")
      .eq("trusted", true);
    ...
    console.log("[list_plugins_for_org] stage: fetch_org_plugins", { org_id });

    const { data: orgPluginData, error: orgPluginError } = await supabase
      .from("org_plugins")
      .select("plugin_id, enabled, rollout_percentage")
      .eq("org_id", org_id)
      .eq("enabled", true);
    ...
    let orgPluginMap: Record<string, { enabled: boolean; rollout_percentage: number }> = {};
    let orgPluginIds: string[] = [];
    ...
    const allPlugins = [...(builtInPlugins || []), ...orgPlugins];
    ...
    for (const p of allPlugins) {
      try {
        ...
        console.log("[list_plugins_for_org] stage: sign_url", { plugin: p.name });
        ...
        results.push({
          ...
          signature: p.signature
            ? btoa(String.fromCharCode(...new Uint8Array(p.signature)))
            : null,
    ...
    console.log("[list_plugins_for_org] stage: complete", { count: results.length });

    return response({ ok: true, data: results });
  } catch (error) {
    console.error("[list_plugins_for_org] stage: crash", { error: String(error) });
    return response({
      ok: false,
