...
function randomVector(dim = 16) {
  const v = Array(dim).fill(0).map(()=>Math.random() - 0.5);
...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: org, error: orgError } = await supabase.from("orgs").select("id, name, claim_secret").eq("claim_secret", claimCode).maybeSingle();
    if (orgError) {
    ...
    const { org_id, name, environment_type, storage_type, capabilities, specs, benchmark_score, force_reclaim } = body;
    if (!name) {
      return jsonResponse({
        ok: false,
    ...
    const { data: existingDevice, error: existingError } = await supabase.from("devices").select("id, name, status, last_seen").eq("org_id", org.id).eq("name", name).maybeSingle();
    if (existingError) {
    ...
    if (existingDevice && !force_reclaim) {
      ...
      const { data: device, error: updateError } = await supabase.from("devices").update({
        ...
        device_vector
      }).eq("id", existingDevice.id).select("id, org_id, name, status").maybeSingle();
      if (updateError) {
        console.error("[register_device] Update error:", updateError.message, updateError.code);
        return jsonResponse({
          ok: false,
          error: "Failed to reclaim device: " + updateError.message
        }, 500, corsHeaders);
      }
      if (!device) {
        return jsonResponse({
          ok: false,
          error: "Device reclaim failed - no device returned"
        }, 500, corsHeaders);
      }
    ...
    const device_vector = randomVector();
    const tokenHash = await hashToken(token);
    const { data: device, error: insertError } = await supabase.from("devices").insert({
      org_id: org.id,
      name,
    ...
    if (insertError) {
      ...
      return jsonResponse({
        ok: false,
        error: "Failed to insert device: " + insertError.message
      }, 500, corsHeaders);
    }
    if (!device) {
      return jsonResponse({
        ok: false,
        error: "Device insert failed - no device returned"
      }, 500, corsHeaders);
    }
    if (!device.status) {
