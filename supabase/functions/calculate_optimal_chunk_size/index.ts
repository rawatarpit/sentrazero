...
serve(async (req) => {
  ...
  try {
    ...
    const avgThroughput =
      benchmarks.reduce((s, b) => s + (b.throughput ?? 0), 0) /
      benchmarks.length;

    const avgLatency =
      benchmarks.reduce((s, b) => s + (b.latency_ms ?? 1), 0) /
      benchmarks.length;

    const efficiency = Math.max(1, avgThroughput / avgLatency);

    const preferred_chunk_size_gb = Math.min(
      8,
      Math.max(0.5, efficiency * 10)
    );

    const { error: updateErr } = await supabase
      .from("devices")
      .update({ preferred_chunk_size_gb })
      .eq("id", device.id)
      .eq("org_id", device.org_id);
    ...
    return new Response(
      JSON.stringify({ ok: true, preferred_chunk_size_gb }),
      { headers: corsHeaders }
    );
  } catch (err) {
    return new Response(
