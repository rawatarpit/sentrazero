...
serve(async (req) => {
  ...
  const supabase = createClient(SUPABASE_URL, SERVICE_KEY, {
    global: { headers: { "x-client-info": "verify_job_lease/1.0.0" } },
  });
  ...
  if (!job_id) {
    return jsonResponse({ ok: false, error: "missing job_id" }, 400, corsHeaders);
  }

  const { data: job, error: jobErr } = await supabase
    .from("agent_jobs")
    .select("id, status, agent_id, lease_expires_at, org_id")
    .eq("id", job_id)
    .maybeSingle();
  ...
  const { data: lease, error: leaseErr } = await supabase
    .from("leases")
    .select("job_id, device_id, lease_expires_at, status")
    .eq("job_id", job_id)
