...
async function processJobWithRetry(
  ...
  job: any,
  retryIndex: number
): Promise<{ id: string; status: string; code?: number; retry_at?: string; error?: string }> {
  try {
    const headers: Record<string, string> = { "Content-Type": "application/json" };
    if (job.headers) for (const [k, v] of Object.entries(job.headers)) headers[k] = v as string;
    ...
    if (success) {
      ...
      return { id: job.id, status: "success", code: res.status };
    } else {
      ...
      return { id: job.id, status: "retry", code: res.status, retry_at: retryAt };
    }
  } catch (err) {
    const nextRetryCount = (job.retry_count ?? 0) + 1;
    const delayMinutes = RETRY_DELAYS[Math.min(nextRetryCount - 1, RETRY_DELAYS.length - 1)];
    ...
    return { id: job.id, status: "retry_error", error: err.message };
  }
}
...
async function batchProcessJobs(
  ...
  for (let i = 0; i < jobs.length; i += CONCURRENT_REQUESTS) {
    ...
    const batchResults = await Promise.all(
      batch.map((job, idx) => processJobWithRetry(supabase, job, i + idx))
    );
...
serve(async (req) => {
  ...
  try {
    const { data: jobs, error } = await supabase
      .from("http_queue")
      .select("id, url, body, headers, processed, status_code, retry_count, idempotency_key")
      .eq("processed", false)
    ...
    const processedJobs = jobs.filter(job => 
      !(job.processed && job.status_code >= 200 && job.status_code < 300)
    );
    
    const alreadyProcessed = jobs.filter(job => 
      job.processed && job.status_code >= 200 && job.status_code < 300
    );
