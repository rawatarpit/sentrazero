...
serve(async (req)=>{
  ...
  const authResult = await authenticateDeviceWithDetails(req, "id, org_id, name, total_cpu_cores, total_memory_gb");
  ...
  try {
    ...
    const { cpu_usage_percent, cpu_cores_free, total_cpu_cores, memory_free_gb, total_memory_gb, io_bandwidth_mb_s, network_latency_ms, gpu_available } = metrics;
    const derived_status = cpu_usage_percent > 90 ? "busy" : memory_free_gb < total_memory_gb * 0.2 ? "busy" : "available";
