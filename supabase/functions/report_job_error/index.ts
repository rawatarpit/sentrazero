...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: job, error: fetchError } = await supabase.from("agent_jobs").select("id, org_id, retry_count, max_retries, status").eq("id", job_id).eq("org_id", authResult.device.org_id).maybeSingle();
    if (fetchError) {
    ...
    }).eq("id", job_id).eq("org_id", authResult.device.org_id);
